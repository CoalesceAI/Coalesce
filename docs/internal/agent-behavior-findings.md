...
Blind tested Haiku agents hitting AgentMail errors with a `support` field. Without any instruction about Coalesce, agents:
...
The agent tried to call the Coalesce support URL but used the AgentMail API key (the only key it had). Got 401. Gave up.

**Solution decided:** Signed URLs — Coalesce gives each org a pre-authenticated base URL. The URL itself contains a token. No separate auth needed. Zero friction for the agent.
