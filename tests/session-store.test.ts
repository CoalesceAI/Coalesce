...
describe('InMemorySessionStore', () => {
  ...
  describe('CRUD operations', () => {
    it('set() and get() stores and retrieves a session', async () => {
    ...
    it('set() can overwrite an existing session', async () => {
  ...
  describe('TTL / expiry behavior', () => {
    it('get() returns undefined for expired session (lazy deletion)', async () => {
      const session = makeSession();
      await store.set('session-1', session);
    ...
    it('get() returns session that has not yet expired', async () => {
      const session = makeSession();
      await store.set('session-1', session);
    ...
    it('get() updates lastAccessedAt on successful retrieval (keeps session alive)', async () => {
      const session = makeSession();
      await store.set('session-1', session);

      // Access at 80ms (before TTL)
      vi.advanceTimersByTime(80);
      const accessed = await store.get('session-1');
    ...
    it('get() returns undefined when accessed past TTL even with previous access', async () => {
      const session = makeSession();
      await store.set('session-1', session);
  ...
  describe('sweep()', () => {
    it('sweep removes all expired sessions', async () => {
    ...
    it('sweep keeps non-expired sessions', async () => {
      const session1 = makeSession({ id: 'session-1' });
      await store.set('session-1', session1);
  ...
  describe('constructor and destroy()', () => {
    it('uses default TTL of 1 hour when no argument provided', async () => {
      const defaultStore = new InMemorySessionStore();
      const session = makeSession();
      await defaultStore.set('s', session);

      // Advance 30 minutes — should still be alive
      vi.advanceTimersByTime(30 * 60 * 1000);
      expect(await defaultStore.get('s')).toBeDefined();
    ...
    it('stores multiple independent sessions', async () => {
      const s1 = makeSession({ id: 'first' });
      const s2 = makeSession({ id: 'second' });
      await store.set('key-1', s1);
      await store.set('key-2', s2);

      expect((await store.get('key-1'))?.id).toBe('first');
      expect((await store.get('key-2'))?.id).toBe('second');
    });
  });
});
